...
import { formatEventDate, formatTime } from "../../utils/dateFormatUtil";
...
function rsvpBadgeColors(status) {
  ...
  if (status === "maybe")
    return { background: RSVP_COLORS.maybe, text: "#000000" };
...
export default function PostCardEventScreen({ route, navigation }) {
  ...
  const fetchMedia = async () => {
    ...
    const { data, error } = await supabase
      .from("event_media")
      .select(
        "id, event, media, media_type, date_added, posted_by, profiles:posted_by(userName, avatar)",
      )
      .eq("event", Number(event.id))
            ...
            },
            {
              ...
              style:
                rsvpValue === "no"
                  ...
                  : undefined,
              labelStyle: rsvpValue === "no" ? { color: "#FFFFFF" } : undefined,
            },
          ]}
        />
      </View>
  ...
  return (
    <View style={styles.screenRoot}>
      ...
      <View
        style={[
          styles.header,
          { paddingTop: insets.top + 8, height: 56 + insets.top },
        ]}
      >
        <TouchableOpacity
            ...
            <FlatList
              ...
              ListEmptyComponent={
                <Text style={styles.attendeesEmptyText}>
                  {isLoadingAvatar ? "Loading..." : "No one invited yet."}
                </Text>
              }
            />
          </Pressable>
...
const styles = StyleSheet.create({
  ...
  header: {
    ...
    alignItems: "center",
    justifyContent: "center",
    paddingHorizontal: 25, // leave room for the back button on the left
    backgroundColor: "#FFF",
  },
  headerBack: {
    position: "absolute",
